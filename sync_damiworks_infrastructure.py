...
AI_SERVICE_DIR = PROJECT_ROOT / "damiworks-ai-service"
INSTANCE_ID = "damiworks_dev"
EXPECTED_EMBEDDING_DIMENSIONS = 768
COMPANY_WEBSITE = "https://damiworks.com"
...
ROUTER_SYSTEM_PROMPT = """You are a strict router for the Dami Works AI automation sales assistant.
...
- RAG_REQUIRED: questions about AI agents, CRM integrations, automation logic, knowledge base behavior, implementation, cases, portfolio, reliability, data safety, objections about managers, or exact Dami Works service facts.
...
MEMORY_SUMMARY_SYSTEM_PROMPT = (
    ...
    "потенциальным клиентом Dami Works. Выдели главное для сделки: ниша бизнеса, "
...
FINAL_SYSTEM_PROMPT = """Ты — ИИ-архитектор и эксперт по автоматизации из Dami Works. Твоя цель — консультировать бизнес и закрывать их на аудит/созвон.
...
KNOWLEDGE_CHUNKS = [
    (
        f"Official Dami Works website: {COMPANY_WEBSITE}."
    ),
    (
        "Интеграция с CRM и таблицами: Мы полностью автоматизируем передачу данных. "
        "ИИ-агенты Dami Works мгновенно отправляют квалифицированные лиды из директа "
    ...
    (
        "Регламент работы Dami Works: стартовый аудит нужен, чтобы понять канал заявок, "
    ...
    (
        "Возражение про менеджеров и гарантии: Dami Works не обещает, что ИИ будет "
...
async def main() -> None:
    print("Syncing Dami Works infrastructure")
    load_environment()
