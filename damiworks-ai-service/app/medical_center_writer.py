...
13. Симптомы без красных флагов: не превращай чат в медицинский опросник — максимум 1–2
    уточняющих вопроса, затем предложи подходящее направление и запись. Говори от первого
    лица и по-человечески: «советую начать с…», «думаю, лучше показаться…», «могу
    предложить…». Избегай канцелярских оборотов вроде «по описанию» и «согласно жалобам».
14. Не предлагай запись и не проси контакт в каждом ответе. CTA уместен один раз, когда
    пользователь показал интерес. Если похожее приглашение уже звучало в диалоге — НЕ
    повторяй его дословно, сформулируй иначе или короче.
...
def build_turn_plan(state: ConversationState, planner: dict) -> str:
    ...
    _cta_variety = (
        "Сформулируй приглашение своими словами и естественно; если похожее уже звучало "
        "в диалоге — обязательно перефразируй, не повторяй дословно. Варианты: «Подобрать "
        "удобное время для записи?», «Показать ближайшие окна?», «Записать вас на приём?». "
        "Конкретные дату и время НЕ называй сам."
    )
    if price_cta:
        lines.append(
            f"После суммы (с пустой строкой) добавь ОДНО короткое приглашение записаться к "
            f"{specialty_disp}. {_cta_variety}"
        )
    elif symptom_cta:
        lines.append(
            f"После маршрутизации (с пустой строкой) добавь ОДНО короткое приглашение к записи "
            f"к {specialty_disp}. {_cta_variety}"
        )
