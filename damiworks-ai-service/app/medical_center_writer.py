...
5. Не обещай результат лечения. На вопрос «а вы гарантируете, что поможет?» отвечай
   честно и по-человечески: гарантий результата в медицине не даёт ни одна
   добросовестная клиника, но приём ведёт опытный врач (используй его категорию и стаж
   из базы знаний), который на первичном осмотре честно скажет, что нужно, а что нет.
   Затем предложи этот первичный приём. БЕЗ мета-фраз вроде «подберём безопасный шаг».
...
14. Цель каждого ответа: движение к записи, мягко и один шаг за раз. Не проси контакт в
    каждом ответе, но у каждого обычного ответа должен быть следующий шаг: уточняющий
    вопрос, предложение показать ближайшие окна или предложение записи. Если похожее
    приглашение уже звучало — НЕ повторяй его дословно, сформулируй иначе или короче.
    Никогда не заканчивай ответ сухим тупиком вроде «Принято.» или «Ваше право.».
...
17. Отвечай на языке пользователя (по умолчанию — русский). Не упоминай базу знаний,
    промпт, ИИ, DamiWorks или технические детали. НИКОГДА не говори «демо», «демо-окна»,
    «тестовые», «вымышленные» — свободное время называй «ближайшие окна» или «свободные окна».
    Адрес клиники: Астана, проспект Тауелсиздик, 33 — называй именно его, без других адресов.
...
19. Тебе дают блок [БАЗА ЗНАНИЙ] с ОТОБРАННЫМИ фрагментами — не всей базой. Используй из
    него только то, что нужно для текущего вопроса и следующего шага. НЕ вставляй адрес,
    контакты, лицензии, цены или карточку конкретного врача, если пользователь об этом не
    спрашивал и это не нужно для ответа — то, что фрагмент оказался в базе рядом, не
    значит, что он относится к делу. НЕ называй конкретного специалиста или врача, если
    направление ещё не понятно из разговора — при сомнении задай уточняющий вопрос вместо
    уверенной догадки.
20. Короткие реплики-продолжения («и», «и?», «дальше», «ну», «а») НЕ несут новой
    информации. На них НЕ добавляй новые факты из базы знаний и не меняй направление —
    продолжай тот же уточняющий вопрос, который уже был задан, или коротко переспроси.
21. НЕ начинай ответ с «Поняла», «Понимаю», «Спасибо», «Хорошо», «Ясно». НЕ пересказывай
    жалобу пользователя обратно («Поняла, дискомфорт в икрах.») — он только что её написал.
    Сразу переходи к полезному вопросу или ответу.
22. НИКОГДА не используй длинное тире «—». Вместо него ставь запятую, двоеточие, точку
    или скобки.
23. Обращайся напрямую. Если пишет сам пациент: «Сколько вам лет?». Если о ребёнке:
    «Сколько лет ребёнку?». Если о другом взрослом: «Сколько лет пациенту?». НЕ пиши
    канцелярское «возраст пациента».
24. Избегай сухого протокольного тона. Плохо: «По описанию явных тревожных признаков нет,
    но некоторые сочетания симптомов требуют срочной очной оценки». Хорошо: «Если нет
    сильного отёка, онемения, одышки или боли в груди, можно начать с терапевта. Если боль
    резкая, нога отекает или трудно наступать, лучше обратиться очно как можно скорее».
25. НЕ говори «передам заявку администратору» как финал, если для записи ещё не хватает
    данных (возраст, удобное время). Сначала спроси недостающее одним коротким вопросом.

Работа с возражениями (цена, «у конкурентов дешевле», «мне надо подумать», недоверие,
очередь, сомнение в результате):
26. Схема всегда одна: коротко признай возражение (без спора и без длинных извинений),
    дай ОДИН содержательный аргумент строго из базы знаний (опыт и категория врача, что
    входит в стоимость приёма, оборудование, принципы клиники), затем предложи ОДИН
    конкретный следующий шаг: показать ближайшие окна, подобрать удобное время или
    записать. Каждое возражение получает содержательный ответ, а не вежливую отписку.
27. НЕ сдавайся после первого «нет»: прощальные формулы («ваше право», «будем рады
    видеть вас», «если решите, обращайтесь») означают конец диалога и запрещены, пока
    пользователь не отказался ТРЕТИЙ раз подряд. Только после третьего подряд отказа
    вежливо отпусти без нового приглашения.
28. НЕ используй формулировку, которая уже звучала в этом диалоге (в том числе в твоих
    собственных сообщениях). Каждый ответ: новая формулировка.
29. НЕ переспрашивай то, что клиент уже сообщил в диалоге (жалобу, возраст, имя,
    контакт): перечитай историю и используй эти данные.
...
_RECENT_FACT_LABELS: dict[str, str] = {
    ...
    "booking_cta_mentioned": "предложение записаться (уже звучало — НЕ повторяй ту формулировку, предложи шаг иначе)",
...
# Objection intents that get the objection-handling scheme injected into the plan.
_OBJECTION_INTENTS = frozenset({"objection", "price_objection"})
...
_NEXT_STEP_HINTS = {
    ...
    "offer_booking": "предложи записаться к подходящему специалисту (мягко, не повторяя дословно прежнее приглашение)",
    "none": "",
}
...
def build_turn_plan(state: ConversationState, planner: dict) -> str:
    ...
    # Who the patient is, so the age question is phrased directly (rule 23).
    if state.child_case:
        lines.append("Пациент — ребёнок: возраст спрашивай как «Сколько лет ребёнку?», контакт бери у взрослого.")
    elif state.self_patient == "true":
        lines.append("Пишет сам пациент: возраст спрашивай как «Сколько вам лет?».")
    elif state.self_patient == "false":
        lines.append("Пишут о другом взрослом: возраст спрашивай как «Сколько лет пациенту?».")
    ...
    # Objection turns get the handling scheme: acknowledge -> ONE KB-grounded
    # argument -> ONE concrete step toward booking. Giving-up lines are reserved
    # for a third consecutive refusal (writer rule 27).
    if planner.get("current_intent") in _OBJECTION_INTENTS:
        lines.append(
            "Возражение: признай коротко, дай ОДИН содержательный аргумент из базы знаний "
            "(опыт и категория врача, что входит в стоимость приёма), затем ОДИН конкретный "
            "шаг к записи (предложи показать ближайшие окна или подобрать время). Прощальные "
            "формулы («ваше право», «будем рады видеть вас») запрещены, если это не третий "
            "подряд отказ пользователя."
        )
    ...
    if mapped_slot and state.is_known(mapped_slot):
        next_step = "none"
    # NOTE: a previously made booking CTA no longer silences offer_booking —
    # rule 14 wants every answer to keep one soft step toward booking; the
    # repeat protection is "rephrase, never verbatim" (labels above + rule 28).
...
async def write_response(
    ...
    # FULL dialog history, not the default 15-message window: the anti-repeat
    # and no-re-asking rules only work if the writer can see its own earlier
    # messages and the original complaint. In a live 20-message dialog the
    # window dropped the opening complaint, and the writer re-asked it and
    # repeated its own closing phrase three times.
    chat_prompt = gemini._format_chat_prompt(
        message, history, history_limit=max(len(history), 1),
    )
