...
- Детализация current_intent:
  ...
  * symptom_description = пользователь описывает жалобы без явного вопроса. Задача — помочь
    ...
    Если жалоба расплывчатая и в базе/разговоре нет чёткого правила маршрутизации для неё —
    recommended_next_step = ask_symptoms (уточни возраст и 1–2 безопасных признака), а НЕ
    offer_booking, и НЕ указывай slots.specialty, пока направление не станет обоснованным.
